...
/*   Updated: 2024/01/24 18:03:19 by mapoirie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*sjoin_noequal(char *s1, char *s2, int i)
{
	...
	if (!s1 && !s2)
		return (NULL);
	dest = malloc(sizeof(char) * ft_strlen(s1) + ft_slen(s2) + 1 + 3);// c'est verifie 1
	if (!dest)
		return (NULL);
	while (s1[++i])
		dest[i] = s1[i];
...
char	*join_in_exp2(t_ms *ms, t_list *exp_tmp, char *cpy_ct, t_cmd *cmd)
{
	...
	if (has_equal(exp_tmp->content))
		join_ct = sjoin_wquote(exp_tmp->content, cpy_ct, 0);// c'est protege
	else
		join_ct = sjoin_noequal(exp_tmp->content, cpy_ct, -1);// c'est protege	
	if (!join_ct)
